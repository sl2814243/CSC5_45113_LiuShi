...
public class ChessPanel extends JPanel implements MouseListener, Runnable {
    ...
    public ChessPanel() {
        ...
        boardImg = Toolkit.getDefaultToolkit().getImage("Background.jpg");
        this.repaint();
        // 添加鼠标监视器
    ...
    public void paint(Graphics g) {
        ...
        g.setFont(new Font("Time New Roman", 0, 14));
        g.drawString("Black Time：" + blackTime, 30, 470);
        g.drawString("White Time：" + whiteTime, 260, 470);
        // 绘制棋盘
        for (int i = 0; i < 15; i++) {
            ...
            while (true) {
                ...
                if (isManAgainst) {
                    if (isBlack) {
                        blackTime--;
                        if (blackTime == 0) {
                            JOptionPane.showMessageDialog(this, "Black Timeout，Game Over!");
                        }
                    } else {
                        whiteTime--;
                        if (whiteTime == 0) {
                            JOptionPane.showMessageDialog(this, "White Timeout，Game Over!");
                        }
                    }
                } else {
                    ...
                    blackTime--;
                    if (blackTime == 0) {
                        JOptionPane.showMessageDialog(this, "User Timeout，Game Over!");
                    }
        ...
        if (x >= 29 && x <= 451 && y >= 29 && y <= 451) {
            ...
            if (allChess[x][y] == 0) {
                ...
                if (isBlack == true) {
                    ...
                    isBlack = false;
                    blackTime = 120;
                    message = "Time to White";
                } else {
                    allChess[x][y] = 2;
                    isBlack = true;
                    whiteTime = 120;
                    message = "Time to Black";
                }
            }
            ...
            this.repaint(); // 绘制棋子
            if (winFlag == true) {
                JOptionPane.showMessageDialog(this, "Game Over," + (allChess[x][y] == 1 ? "Black" : "White") + "Win!");
                canPlay = false;
            }
        } 
    }
